...
struct TaxCalendarEngineTests {
    ...
    func settingsFallbacks() {
        ...
        #expect(BackupService.currentVersion == 14)
        #expect(BackupService.backedUpSettingsKeys.contains(AppSettingsKeys.numberPatternPRO))
        #expect(BackupService.backedUpSettingsKeys.contains(AppSettingsKeys.reminderEmailsEnabled))
        #expect(BackupService.backedUpSettingsKeys.contains(AppSettingsKeys.demandInterestRate))
